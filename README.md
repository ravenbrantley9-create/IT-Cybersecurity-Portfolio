Welcome to my IT and cybersecurity portfolio!

I am currently building my technical skills through hands-on computer projects, IT coursework, troubleshooting, and cybersecurity training.

This portfolio documents my progress and demonstrates the technical skills I am developing as I prepare for a career in IT and cybersecurity.

---

## 🎓 Current Training

- Google IT Support Professional Certificate
- Computer Hardware & Architecture
- Operating Systems
- Networking Fundamentals
- Linux Fundamentals
- Cybersecurity Fundamentals

### 📚 IT Support Coursework & Labs

- 🖥️ [Computer Hardware Fundamentals](IT-Support-Labs/Computer-Hardware/)
---

## 🖥️ Hands-On IT Projects

### 🔧 [Dell OptiPlex 760 Troubleshooting & Rebuild](Hands-On-Projects/Dell-OptiPlex-760/)

Hands-on computer hardware troubleshooting project using a Dell OptiPlex 760.

**Skills Practiced:**

- Identifying internal computer components
- Opening and safely working inside a desktop computer
- Diagnosing POST/startup problems
- Testing RAM modules individually
- Working with RAM slots
- Removing and disconnecting a hard drive
- Disconnecting and removing an optical/DVD drive
- Checking internal power and data connections
- Running built-in hardware diagnostics
- Using beep codes and system behavior during troubleshooting
- Documenting troubleshooting steps and results



---

## 🔐 Cybersecurity Labs

Cybersecurity labs will be added as I progress through my training.

Topics will include:

- Linux
- Networking
- Security fundamentals
- System administration
- Threat detection
- Incident response

---

## 🧠 Technical Skills

**Hardware**
- Desktop computer components
- RAM installation and troubleshooting
- Storage devices
- Basic PC diagnostics

**Operating Systems**
- Windows
- Linux — currently learning

**Networking**
- Networking fundamentals — currently learning

**Cybersecurity**
- Security fundamentals — currently learning

---

## 📚 Certifications & Education

### Google IT Support Professional Certificate

🚧 In Progress

---

## 🎯 Career Goal

My goal is to build a strong foundation in information technology and gain professional experience in IT support while continuing to develop the skills needed to transition into cybersecurity.

---

## 📈 Portfolio Status

🚧 **Actively Building**

This portfolio will continue to grow as I complete new courses, labs, certifications, and hands-on projects.
